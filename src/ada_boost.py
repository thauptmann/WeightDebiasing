...
from utils.training import (
    cv_bootstrap_prediction,
    weighted_auc_prediction,
)
...
def ada_boost_debiasing(
    df, columns, dataset, number_of_splits=5, number_of_iterations=20, flip_rate=0.1
):
    aurocs = list()
    boosted_aurocs = list()
    error_rates = list()
    ...
    asam = average_standardised_absolute_mean_distance(
        df, columns, weights=np.ones(non_representative_size)
    )
    ...
    for i in tqdm(range(number_of_iterations)):
        prediction_n, clf = cv_bootstrap_prediction(
            scaled_df, number_of_splits, columns
        )
        ...
        predicted_classes = np.where(
            flip, (predicted_classes + 1) % 1, predicted_classes
        )
        ...
        for alpha, classifier in zip(
            alpha_list[half_list:], classifier_list[half_list:]
        ):
            weighted_prediction = (
                classifier.predict_proba(scaled_df[columns])[:, 1] * alpha
            )
        ...
        weighted_mmd = maximum_mean_discrepancy_weighted(
            scaled_N[columns], scaled_R[columns], new_weights.values
        )
        mmds.append(weighted_mmd)
        weighted_asams = average_standardised_absolute_mean_distance(
            df, columns, weights=new_weights
        )
    ...
    plot_line(
        boosted_aurocs, visualisation_path, "Boosted AUROC", plot_random_line=True
    )
    ...
    weighted_asams = average_standardised_absolute_mean_distance(
        scaled_df, columns, best_weights
    )
